...
import { GameRenderer, keyToCoord, previewPlacement, placeShipDOM, removeShipDOM, clearPreview, offsetCoord, } from "./ts/GameRenderer";
...
const animTime = parseFloat(curRootStylings.getPropertyValue("--anim-time"));
const animDelay = parseFloat(curRootStylings.getPropertyValue("--anim-delay"));
// Overlay component and modal elements.
const overlay = document.getElementById("overlay");
const rulesModal = document.getElementById("rules");
const startModal = document.getElementById("start");
const turnModal = document.getElementById("turn");
...
const modals = Array.from(document.querySelectorAll(".overlay__modal"));
for (const modal of modals) {
    (_a = modal.querySelector("#modal__exit")) === null || _a === void 0 ? void 0 : _a.addEventListener("click", () => {
...
const buttons = Array.from(document.querySelectorAll("button[id$='_btn']"));
        ...
        case "modal__start_btn":
            button.addEventListener("click", async (e) => {
                ...
                if (validateForm(startForm)) {
                    const modal = button.closest(".overlay__modal");
                    ...
                    console.log("Player names:", playerName1, `${playerName2 !== null && playerName2 !== void 0 ? playerName2 : AI_NAME}`);
                    ...
                    if (mainContainer) {
                        ...
                        const gameController = new GameController(players[0], players[1], renderer);
                    }
                }
            });
            break;
    ...
    if (!e.repeat) {
        switch (e.key) {
            case "Escape":
                ...
                const openModal = modals.find((modal) => !modal.classList.contains("--hidden"));
                if (openModal)
                    toggleOverlay({ modalEl: openModal });
        }
    }
    else {
...
const startForm = document.querySelector(".modal__form");
const inputs = Array.from(startForm === null || startForm === void 0 ? void 0 : startForm.querySelectorAll("input[type='text']"));
...
function toggleOverlay({ modalEl, overlayConfig, modalConfig, }) {
...
function validateForm(form) {
    ...
    const playerNameInputs = Array.from(form.querySelectorAll("input[type='text'"));
    ...
    if (gameMode == HUMAN_MODE) {
        if (input.type === "text") {
            ...
            if (input.validity.tooLong ||
                ...
                input.setCustomValidity("Name must be between 2 and 16 characters in length");
                inputValid = false;
                console.error(`Input ${input.id} is empty, or too short.`);
            }
            // Check if player1 name is same as player2
            let player1Name = document.getElementById("input_name1");
            let player2Name = document.getElementById("input_name2");
            if (player1Name &&
                player2Name &&
...
function dragShip(e, player) {
    ...
    const shipSegment = e.target;
    const segmentIndex = Array.from(ship.children).indexOf(shipSegment);
    const boardDiv = document.querySelector(".game__board");
    const cellElements = Array.from(boardDiv.querySelectorAll("[data-cell]"));
    const shipContainer = document.querySelector(".ship__container");
    let shipPlaced = false;
    let styledCells = [];
    let data = {
        shipName: ship.dataset.name,
        length: Number(ship.dataset.length),
        startOrientation: ship.dataset.orientation,
        curOrientation: ship.dataset.orientation,
        clickMouseX: e.clientX,
        clickMouseY: e.clientY,
        previouslyPlaced: ship.dataset.placed,
        startCoord: undefined, // The starting coordinate of a placed ship.
        curCoord: undefined, // The coord at which a placed ship was grabbed.
    };
    let cellOffset = segmentIndex *
    ...
    if (curKey !== undefined && !Number.isNaN(curKey)) {
        ...
        data.startCoord = keyToCoord(startKey);
    }
    ...
    const offsetParent = ship.offsetParent;
    ...
    if (data.previouslyPlaced === "true") {
        ...
        const cellDiv = document.elementFromPoint(curX, curY);
        if (cellDiv) {
            const curCoord = keyToCoord(Number(cellDiv.dataset.cell));
            // Get the starting coordinate (may be negative x or y value).
            const startCoord = data.curOrientation === "horizontal"
                ? [curCoord[0] - segmentIndex, curCoord[1]]
                : [curCoord[0], curCoord[1] - segmentIndex];
            styledCells.push(...previewPlacement(startCoord, curCoord, player, data.shipName, data.length, data.curOrientation, boardDiv));
        }
    }
    const move = (e) => {
        curX = e.clientX;
    ...
    const release = (e) => {
        ...
        let el = elements.find((el) => el.classList.contains("board__cell"));
        ship.style.visibility = "";
        // Get board cell information.
        const cellKey = Number(el === null || el === void 0 ? void 0 : el.dataset.cell);
        const cellCoord = keyToCoord(cellKey);
        ...
        if (shipPlaced) {
            ...
            player.placeShip(data.shipName, data.startCoord, data.startOrientation);
            placeShipDOM(boardDiv, ship, data.startCoord, data.startOrientation);
        }
        else {
    ...
    const changeOrientation = (e) => {
        if (!e.repeat) {
            switch (e.key) {
                case "r":
                    let newOrientation = data.curOrientation === "horizontal"
                        ? "vertical"
                        : "horizontal";
                    ...
                    newOrientation === "horizontal"
                        ? ship.classList.replace("ship__unit--vertical", "ship__unit")
                        : ship.classList.replace("ship__unit", "ship__unit--vertical");
                    ship.style.transform =
    ...
    for (const cellEl of cellElements) {
        cellEl.onpointerenter = () => {
            ...
            let startCoord = offsetCoord(curCoord, segmentIndex, data.curOrientation);
            styledCells.push(...previewPlacement(startCoord, curCoord, player, data.shipName, data.length, data.curOrientation, boardDiv));
        };
...
export { dragShip, modalConfigs, overlayConfigs, animDelay, HUMAN_MODE, AI_MODE, };
